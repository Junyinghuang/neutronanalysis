...
struct SlcNumSizeStruct {
    ...
    bool operator==(const SlcNumSizeStruct &o) const {
        return SlcNum == o.SlcNum && SlcSize == o.SlcSize;
    }

    bool operator<(const SlcNumSizeStruct &o)  const {
        return SlcNum < o.SlcNum || (SlcNum == o.SlcNum && SlcSize < o.SlcSize);
    }
};
...
namespace dune{
    ...
    private:
        ...
        //bool Scanning(std::map<gridStruct, std::vector<hitTagStruct>>::iterator & gItr, int cid, double pt);
...
*/
 ...
 /*     
        ...
        std::vector<double> SptHitPeakTime;
...
*/
    ...
    neutronana::neutronana(fhicl::ParameterSet const& pset) :
        ...
        fPandoraTag               (pset.get<std::string>("PandoraLabel","")    ),
...
*/
    ...
    void neutronana::analyze( const art::Event& evt){  
        ...
        art::Handle< std::vector<recob::SpacePoint> > pointsListHandle; // to get information about the Spacepoints //from reco 3d
        ...
        art::Handle< std::vector<recob::SpacePoint> > pandoraPointsHandle; // to get information about the Spacepoints from Pandora
        std::vector<art::Ptr<recob::SpacePoint> > pandoraPointsList;
        if(evt.getByLabel(fPandoraTag, pandoraPointsHandle)){
          art::fill_ptr_vector(pandoraPointsList, pandoraPointsHandle);
        }
...
*/
        ...
        ///art::FindManyP<recob::Hit> hitsFromSpsPandora(pandoraPointsHandle, evt, fPandoraTag); //to associate space point from pandora to hit
...
*/
        ...
        for (int i = 0; i < (int) pointsList.size(); ++i){
            ...
            {
                SptClusterTag.push_back(itr->second.SlcNum); //Cluster space point
                SptClusterSize.push_back(itr->second.SlcSize); 
                //TAG = 1; //Bad Hit
            } else {
...
/*
    ...
    bool neutronana::Scanning(std::map<gridStruct, std::vector<hitTagStruct>>::iterator & gItr, int cid, double pt) {
